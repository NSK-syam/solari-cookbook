...
export interface ApprovalScene extends SceneBase { kind: "approval"; vendorName: string; appointment: string; price: string; serviceType: string; actionLabel: "Approve rescue"; phase: RescuePhase; statusHeading: string; statusCopy: string; simulated: boolean }
...
export function projectInvestigation(view: ClosingRescueView, story: StorySnapshot): DocumentaryCut {
  ...
  const scenes: DocumentaryCut["scenes"] = [
    ...
    { kind: "approval", chapter: 6, label: "Human checkpoint", truthLabel: "Synthetic vendor data · simulated action", vendorName: vendor?.vendor_name ?? "No qualifying vendor", appointment: vendor ? formatTimestamp(vendor.appointment_at) : "Manual escalation required", price: formatMoney(vendor?.price_cents ?? 0), serviceType: vendor?.service_type ?? "No appointment available", actionLabel: "Approve rescue", phase, statusHeading, statusCopy, simulated: true }
  ];
